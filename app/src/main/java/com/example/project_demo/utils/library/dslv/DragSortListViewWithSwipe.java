...
package com.example.project_demo.utils.library.dslv;
...
import com.example.project_demo.utils.library.swipelistview.SwipeListView;
 ...
 *
 */
public class DragSortListViewWithSwipe extends SwipeListView {
    ...
    public DragSortListViewWithSwipe(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            ...
            if (useDefault) {
                ...
                int removeMode = a.getInt(
                        ...
                        DragSortControllerWithSwipe.FLING_REMOVE);
                ...
                int dragInitMode = a.getInt(
                        ...
                        DragSortControllerWithSwipe.ON_DOWN);
                ...
                controller = new DragSortControllerWithSwipe(
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView != null) {
                ...
                child = mAdapter.getView(position, oldChild, DragSortListViewWithSwipe.this);
                if (child != oldChild) {
                ...
                child = mAdapter.getView(position, null, DragSortListViewWithSwipe.this);
            ...
            if(child != null){
                ViewGroup vg = (ViewGroup) v.findViewById(swipeFrontView);
                View lastChild = vg.getChildAt(vg.getChildCount()-1);
                boolean hasDrager = false;
                if(lastChild instanceof ImageView && lastChild.getTag().equals("DragButton")){
                    hasDrager = true;
                }
                if(_useDrag){
                    if(!hasDrager){
                        ...
                        img.setTag("DragButton");
                        AbsListView.LayoutParams params = new AbsListView.LayoutParams(AbsListView.LayoutParams.WRAP_CONTENT, AbsListView.LayoutParams.MATCH_PARENT);
                        img.setLayoutParams(params);
                        img.setImageDrawable(dragHandleDrawable);
                        img.setId(controller.getDragHandleId());
                        vg.addView(img);
                    }
                }else{
                    if(hasDrager)vg.removeViewAt(vg.getChildCount()-1);
                }
            }
    ...
    private void measureItem(View item) {
        ...
        int hspec;
        if (lp.height > 0) {
            hspec = View.MeasureSpec.makeMeasureSpec(lp.height, View.MeasureSpec.EXACTLY);
        } else {
            hspec = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
        }
    ...
    public interface FloatViewManager {
         ...
         * and the offset provided in {@link DragSortListViewWithSwipe#startDrag}.
     ...
     * {@link DragSortListViewWithSwipe#setDragScrollProfile(DragScrollProfile)} to
     * set custom profile.
     * 
     * @author heycosmo
     *
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                DragSortListViewWithSwipe.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
        	if(_useDrag){
                ...
                DragSortListViewWithSwipe.this.layoutChildren();
                invalidate();
    ...
    /*****    for swipe    *****/
    DragSortControllerWithSwipe controller;
    private boolean _useDrag = true;
    private Drawable dragHandleDrawable;
    private int lastSwipeMode = -1;
    
    public void setDragHandleDrawable(Drawable d){
    	dragHandleDrawable = d;
    }
    
    public DragSortControllerWithSwipe getmController() {
		return controller;
	}

	public void setmController(DragSortControllerWithSwipe mController) {
    ...
    public void useDrag(){
    	...
    	lastSwipeMode = touchListener.getSwipeMode();
    	setSwipeMode(SWIPE_MODE_NONE);
    	if(controller.getDragHandleId() > 0 && dragHandleDrawable != null){
	    	int count = getCount();
	    	for (int i=0;i<count;i++){
	    		ViewGroup v = (ViewGroup) getChildAt(i);
                if(v == null)continue;
	    		...
	    		ImageView img = new ImageView(getContext());
                img.setTag("DragButton");
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(AbsListView.LayoutParams.WRAP_CONTENT, AbsListView.LayoutParams.MATCH_PARENT);
